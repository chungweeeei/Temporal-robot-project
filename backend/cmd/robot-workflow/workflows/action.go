...
func RobotActionWorkflow(ctx workflow.Context, req map[string]interface{}) (string, error) {
	...
	// switch req.Action {
	// case StandUp:
	// 	future := workflow.ExecuteActivity(childCtx, ra.Standup)
	// 	err = future.Get(ctx, &result)
	// case SitDown:
	// 	future := workflow.ExecuteActivity(childCtx, ra.Sitdown)
	// 	err = future.Get(ctx, &result)
	// case Move:
	// 	future := workflow.ExecuteActivity(childCtx, ra.Move, *req.MoveTarget)
	// 	err = future.Get(ctx, &result)
	// default:
	...
	// 		recoveryErr := workflow.ExecuteActivity(disconnectedCtx, ra.Sitdown).Get(disconnectedCtx, &recoveryResult)
